...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!token) {
    return next(new AppError("You don't have permissions to use this", 401));
  }
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  try {
    const url = `http://${req.get('host')}/api/v1/users/resetpassword/${resetToken}`;
...
exports.passwordResetToken = catchAsync(async (req, res, next) => {
  ...
  user.passwordResetToken = undefined;
  user.passwordResetExpires = undefined;
  user.changedPasswordAfter = Date.now() + 10000;
  await user.save({ validateBeforeSave: false });
